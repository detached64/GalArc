...
internal abstract class QlieEncryption
{
    ...
    public static unsafe byte[] GetCommonKey(byte[] data)
    {
        ...
        fixed (byte* p = key)
        {
            ...
            {
                int temp = (i % 3) != 0 ? (i + 7) * -(i + 3) : (i + 7) * (i + 3);
                key32[i] = (uint)temp;
            }
    ...
    public static QlieEncryption Create(int version, byte[] game_key)
    {
        return version switch
        {
            10 => new Encryption10(),
            20 => new Encryption20(),
            30 => new Encryption30(game_key),
            31 => new Encryption31(),
            _ => throw new ArgumentException("Unknown version", nameof(version)),
        };
    }
    ...
    public static QlieEncryption Create(string version)
    {
        return version switch
        {
            "1.0" => new Encryption10(),
            "2.0" => new Encryption20(),
            "3.1" => new Encryption31(),
            _ => throw new ArgumentException("Unknown or not implemented version.", nameof(version)),
        };
    }
}
...
internal class Encryption30(byte[] data) : QlieEncryption
{
    public override int Version => 30;

    private readonly byte[] GameKey = data;
    ...
    public override void DecryptEntry(byte[] data, QlieEntry entry)
    {
        byte[] key_file = entry.CommmonKey;
        ...
        QlieMersenneTwister mt = new(seed);
...
internal class Encryption31 : QlieEncryption
{
    ...
    public override void DecryptEntry(byte[] data, QlieEntry entry)
    {
        switch (entry.IsEncrypted)
        {
            ...
            case 2:
                ...
                break;
            default:
                throw new InvalidDataException("Invalid encryption flag");
        }
    }
    ...
    private void DecryptV1(byte[] data, string name, uint key)
    {
        ...
        v2 = 9 * (v2 & 0xFFFFFF);
        byte[] table = [.. CreateTable(0x40, v2, true).SelectMany(BitConverter.GetBytes)];
    ...
    private void DecryptV2(byte[] data, string name, uint key, byte[] common_key)
    {
        ...
        v2 = 13 * (v2 & 0xFFFFFF);
        byte[] table = [.. CreateTable(0x40, v2, false).SelectMany(BitConverter.GetBytes)];
        int round = data.Length >> 3;
        if (round > 0)
        {
            unsafe
    ...
    private static uint[] CreateTable(int length, uint value, bool isV1)
    {
