using CommunityToolkit.Mvvm.ComponentModel;
using GalArc.I18n;
using GalArc.Infrastructure.Logging;
using GalArc.Infrastructure.Progress;
using GalArc.Models.Formats.Commons;
using GalArc.Models.Utils;
using System.Collections.Generic;
using System.IO;
using System.Text;
using System.Text.RegularExpressions;

namespace GalArc.Models.Formats.Majiro;

internal partial class ARC : ArcFormat, IUnpackConfigurable, IPackConfigurable
{
    public override string Name => "ARC";
    public override string Description => "Majiro Archive";
    public override bool CanWrite => true;

    private MajiroARCUnpackOptions _unpackOptions;
    public ArcOptions UnpackOptions => _unpackOptions ??= new MajiroARCUnpackOptions();

    private MajiroARCPackOptions _packOptions;
    public ArcOptions PackOptions => _packOptions ??= new MajiroARCPackOptions();

    private const string Magic = "MajiroArcV{0}.000\0";
    private const string MagicPattern = "^MajiroArcV(\\d)\\.000\\x00$";
    private const string ScriptMagicDec = "MajiroObjV1.000\0";
    private const string ScriptMagicEnc = "MajiroObjX1.000\0";

    private class MajiroEntry : Entry
    {
        public ulong Hash { get; set; }
    }

    public override void Unpack(string filePath, string folderPath)
    {
        using FileStream fs = File.OpenRead(filePath);
        using BinaryReader br = new(fs);
        string magic = Encoding.ASCII.GetString(br.ReadBytes(16));
        Match match = MajiroMagicRegex().Match(magic);
        if (!match.Success)
        {
            throw new InvalidArchiveException();
        }
        int version = int.Parse(match.Groups[1].Value);
        if (version < 1 || version > 3)
        {
            throw new InvalidVersionException(InvalidVersionType.Unknown);
        }
        Logger.ShowVersion("arc", version);
        fs.Position = 16;
        int fileCount = br.ReadInt32();
        uint nameOffset = br.ReadUInt32();
        uint dataOffset = br.ReadUInt32();
        int indexLength = (version + 1) * 4 * fileCount;
        if (version == 1)
        {
            indexLength += 8;
        }
        List<MajiroEntry> entries = [];

        using MemoryStream ms = new(br.ReadBytes(indexLength));
        using (BinaryReader indexReader = new(ms))
        {
            for (int i = 0; i < fileCount; i++)
            {
                MajiroEntry entry = new();
                entry.Hash = version < 3 ? indexReader.ReadUInt32() : indexReader.ReadUInt64();
                entry.Offset = indexReader.ReadUInt32();
                if (version > 1)
                {
                    entry.Size = indexReader.ReadUInt32();
                }
                entry.Name = br.ReadCString();
                entries.Add(entry);
            }
            if (version == 1)
            {
                MajiroEntry lastEntry = new();
                lastEntry.Hash = indexReader.ReadUInt32();
                lastEntry.Offset = indexReader.ReadUInt32();
                entries.Add(lastEntry);
            }
        }
        ProgressManager.SetMax(fileCount);

        for (int i = 0; i < fileCount; i++)
        {
            byte[] data;
            if (version == 1)
            {
                data = br.ReadBytes((int)(entries[i + 1].Offset - entries[i].Offset));
            }
            else
            {
                fs.Position = entries[i].Offset;
                data = br.ReadBytes((int)entries[i].Size);
            }
            if (_unpackOptions.DecryptScripts && Path.GetExtension(entries[i].Name) == ".mjo")
            {
                Logger.Debug(MsgStrings.Decrypting, entries[i].Name);
                DecryptScript(data);
            }
            File.WriteAllBytes(Path.Combine(folderPath, entries[i].Name), data);
            data = null;
            ProgressManager.Progress();
        }
    }

    public override void Pack(string folderPath, string filePath)
    {
        int version = _packOptions.Version;
        using FileStream fw = File.Create(filePath);
        using BinaryWriter bw = new(fw);
        DirectoryInfo d = new(folderPath);
        FileInfo[] files = d.GetFiles();
        int fileCount = files.Length;
        ProgressManager.SetMax(fileCount);
        // write header
        bw.Write(Encoding.ASCII.GetBytes(string.Format(Magic, version)));
        bw.Write(fileCount);
        uint nameOffset = 28 + (uint)((version + 1) * 4 * fileCount);
        if (version == 1)
        {
            nameOffset += 8;
        }
        uint dataOffset = 0;
        bw.Write(nameOffset);
        bw.Write(dataOffset);
        // collect entries
        List<MajiroEntry> entries = [];
        foreach (FileInfo file in files)
        {
            MajiroEntry entry = new();
            entry.Name = file.Name;
            entry.Path = file.FullName;
            entry.Size = (uint)file.Length;
            entry.Hash = version < 3
                ? Crc32.Calculate(ArcEncoding.Shift_JIS.GetBytes(entry.Name))
                : CalculateHash(ArcEncoding.Shift_JIS.GetBytes(entry.Name));
            entries.Add(entry);
        }
        entries.Sort((a, b) => a.Hash.CompareTo(b.Hash));
        // write name
        fw.Position = nameOffset;
        foreach (MajiroEntry entry in entries)
        {
            bw.Write(ArcEncoding.Shift_JIS.GetBytes(entry.Name));
            bw.Write('\0');
        }
        // write data
        dataOffset = (uint)fw.Position;
        foreach (MajiroEntry entry in entries)
        {
            byte[] data = File.ReadAllBytes(entry.Path);
            if (_packOptions.EncryptScripts && Path.GetExtension(entry.Name) == ".mjo")
            {
                Logger.Debug(MsgStrings.Decrypting, entry.Name);
                EncryptScript(data);
            }
            bw.Write(data);
            data = null;
        }
        uint maxOffset = (uint)fw.Position;
        // write index
        fw.Position = 24;
        bw.Write(dataOffset);
        foreach (MajiroEntry entry in entries)
        {
            if (version < 3)
            {
                bw.Write((uint)entry.Hash);
            }
            else
            {
                bw.Write(entry.Hash);
            }
            bw.Write(dataOffset);
            if (version > 1)
            {
                bw.Write(entry.Size);
            }
            dataOffset += entry.Size;
            ProgressManager.Progress();
        }
        if (version == 1)
        {
            bw.Write(0);
            bw.Write(maxOffset);
        }
    }

    private void DecryptScript(byte[] data)
    {
        if (Encoding.ASCII.GetString(data, 0, 16) != ScriptMagicEnc)
        {
            return;
        }
        data[9] = (byte)'V';
        int start = 0x28, end = data.Length - start;
        for (int i = 0; i < end; i++)
        {
            data[i + start] ^= key[i % key.Length];
        }
    }

    private void EncryptScript(byte[] data)
    {
        if (Encoding.ASCII.GetString(data, 0, 16) != ScriptMagicDec)
        {
            return;
        }
        data[9] = (byte)'X';
        int start = 0x28, end = data.Length - start;
        for (int i = 0; i < end; i++)
        {
            data[i + start] ^= key[i % key.Length];
        }
    }

    private static ulong CalculateHash(byte[] bytes)
    {
        const ulong POLY = 0x85E1C3D753D46D27;
        uint a = 0xFFFFFFFF;
        uint b = 0xFFFFFFFF;

        ulong[] table = new ulong[256];
        for (int i = 0; i < 256; i++)
        {
            ulong hash = (ulong)i;
            for (int j = 0; j < 8; j++)
            {
                if ((hash & 1) != 0)
                {
                    hash ^= POLY;
                }
                hash >>= 1;
            }
            table[i] = hash;
        }

        for (int i = 0; i < bytes.Length; i++)
        {
            int index = (byte)a ^ bytes[i];
            ulong temp = (((ulong)b << 32) | a) >> 8;
            a = (uint)(temp ^ table[index]);
            b = (uint)((temp ^ table[index]) >> 32);
        }
        return ~(((ulong)b << 32) | a);
    }

    private readonly byte[] key =
    [
        
    ];

    [GeneratedRegex(MagicPattern)]
    private static partial Regex MajiroMagicRegex();
}

internal partial class MajiroARCUnpackOptions : ArcOptions
{
    [ObservableProperty]
    private bool decryptScripts = true;
}

internal partial class MajiroARCPackOptions : ArcOptions
{
    [ObservableProperty]
    private bool encryptScripts = true;
    [ObservableProperty]
    private int version = 1;
    [ObservableProperty]
    private IReadOnlyList<int> versions = [1, 2, 3];
}
