...
namespace ArcFormats.Ethornell
{
    ...
    {
        private uint dec_count;
        private uint unpacked_size;
        private BitStream input;
        private byte[] output;

        public DscDecoder(byte[] data)
        {
            this.key = BitConverter.ToUInt32(data, 16);
            this.unpacked_size = BitConverter.ToUInt32(data, 20);
            this.dec_count = BitConverter.ToUInt32(data, 24);
            this.input = new BitStream(new MemoryStream(data), BitStreamMode.Read, BitStreamEndianness.Msb);
            this.output = new byte[this.unpacked_size];
            input.BaseStream.Position = 0x20;
        }

        private void CreateTree(DscNode[] hnodes, DscCode[] hcode, int node_count)
        {
            ...
            for (int n = 0; n < node_count;)
            {
                ...
                {
                    var node = new DscNode { IsParent = true };
        ...
        private struct DscNode      // must be 'struct' here
        {
        ...
        private struct DscCode      // same here
        {
            public ushort Code;
            public ushort Depth;
        }
    ...
    internal class SdcDecoder : DecryptionBase
    {
        ...
        private ushort check_xor;
        private byte[] input;

        public SdcDecoder(byte[] data)
        {
            this.key = BitConverter.ToUInt32(data, 16);
            this.size = BitConverter.ToInt32(data, 20);
            this.unpacked_size = BitConverter.ToUInt32(data, 24);
            this.check_sum = BitConverter.ToUInt16(data, 28);
            this.check_xor = BitConverter.ToUInt16(data, 30);
