...
namespace ArcFormats.Siglus
{
    internal class SiglusUtils
    {
        ...
        public static void Decrypt(byte[] data, int type = 0)
        {
            for (int i = 0; i < data.Length; i++)
            {
                data[i] ^= DecryptKeys[type, i % 256];
            }
        }

        public static byte[] Decompress(byte[] input, uint size)
        {
            int inIndex = 0;
            int outIndex = 0;
            ...
            {
                short s = 8;
                short byteVal = input[inIndex++];

                while (s > 0 && outIndex < size)
                {
                    if ((byteVal & 1) != 0)
                    {
                    ...
                    {
                        ushort data = BitConverter.ToUInt16(input, inIndex);
                        inIndex += 2;
                        ...
                        {
                            output[outIndex] = output[outIndex - data];
                            outIndex++;
                        }
                    }
                    s--;
